...
import { postDb } from '../../database.js';
...
router.post('/:id/like', authMiddleware, async (req, res) => {
  try {
    ...
    await postDb.toggleLike(req.params.id, userId);
    res.json({ message: 'OK' });
  } catch (error: any) {
...
router.post('/:id/share', authMiddleware, async (req, res) => {
  try {
    ...
    const post = await postDb.findById(req.params.id);
    if (!post) return res.status(404).json({ error: 'Bài viết không tồn tại' });
    await postDb.share(req.params.id, userId);
    res.json({ message: 'OK' });
  } catch (error: any) {
...
router.post('/:postId/comments/:commentId/like', authMiddleware, async (req, res) => {
  try {
    const { userId } = (req as any).user;
    await postDb.toggleCommentLike(req.params.commentId, userId);
    res.json({ message: 'OK' });
  } catch (error: any) {
