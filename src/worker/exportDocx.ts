...
export async function createPatctcDocx(data: any): Promise<ArrayBuffer> {
...
export function makePatctcDocxFilename(data: any): string {
  const raw = (data?.soVb || 'export').toString().replace(/[^a-zA-Z0-9_-]/g, '_');
  return `PATCTC_${raw}.docx`;
}
